...
for itrial = 1:numTrials
    es.contrast(es.trialID==itrial & es.traj~=0)    = VRdata.TRIAL.trialContr(trialIDs(itrial));
    es.start(es.trialID==itrial & es.traj~=0)       = VRdata.TRIAL.trialStart(trialIDs(itrial));
    es.gain(es.trialID==itrial & es.traj~=0)        = VRdata.TRIAL.trialGain(trialIDs(itrial));
    es.blanks(es.trialID==itrial & es.traj~=0)      = VRdata.TRIAL.trialBlanks(trialIDs(itrial));
    es.active(es.trialID==itrial & es.traj~=0)      = VRdata.TRIAL.trialActive(trialIDs(itrial));
    es.rewardPos(es.trialID==itrial & es.traj~=0)   = VRdata.TRIAL.trialRewPos(trialIDs(itrial));
    es.outcome(es.trialID==itrial & es.traj~=0)     = VRdata.TRIAL.trialOutcome(trialIDs(itrial));
    es.roomLength(es.trialID==itrial & es.traj~=0)  = VRdata.TRIAL.trialRL(trialIDs(itrial));
end
