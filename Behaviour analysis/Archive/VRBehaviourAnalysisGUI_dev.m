function varargout = VRBehaviourAnalysisGUI_dev(varargin)
...
if strcmp(hostname,'saleem01')
    DIRS.ball = 'Y:\Saleem Lab\Data\Behav';
else
    DIRS.ball = 'S:\Saleem Lab\Data\Behav';
end

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @VRBehaviourAnalysisGUI_dev_OpeningFcn, ...
    'gui_OutputFcn',  @VRBehaviourAnalysisGUI_dev_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
a = varargin;
...
% --- Executes just before VRBehaviourAnalysisGUI_dev is made visible.
function VRBehaviourAnalysisGUI_dev_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to VRBehaviourAnalysisGUI_dev (see VARARGIN)

% Choose default command line output for VRBehaviourAnalysisGUI_dev
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes VRBehaviourAnalysisGUI_dev wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = VRBehaviourAnalysisGUI_dev_OutputFcn(hObject, eventdata, handles)
global DIRS
...
expt_list = get(handles.popupmenu1,'String');
curr_iexp = get(handles.popupmenu1,'Value');
iexp = str2num(expt_list(curr_iexp,:));

processCurrExp(handles);

function processCurrExp(handles);
global info;
global animal;
global iseries
global iexp
...
curr_iexp = curr_iexp + 1;
if curr_iexp>length(expt_list)
    curr_iexp = curr_iexp - 1;
else
    iexp = str2num(expt_list(curr_iexp,:));
end
set(handles.popupmenu1,'Value', curr_iexp)    
processCurrExp(handles);
